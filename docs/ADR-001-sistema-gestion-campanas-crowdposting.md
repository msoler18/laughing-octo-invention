...
### Fase 2 — IA aplicada a operaciones

**Clasificación y enriquecimiento:** LLM analiza bio, handle y categorías declaradas para asignar o validar categorías en la taxonomía de dos niveles. Ejecución en batch asíncrono, nunca en tiempo de consulta.

**Scoring de fit:** combinación de `tier`, `engagement_quality`, historial de campañas, categoría primaria, ciudad y `consistency_score`. Produce valor 0-100 por creador/campaña. Es explicable: el sistema muestra qué variables pesaron en el score.

**RAG conversacional:**
- Texto compuesto vectorizado por creador: nombre + handle + ciudad + categorías + tier + engagement_quality + bio_text
- Metadata estructurada (followers, engagement_rate, ciudad, tier) como filtros duros previos al ranking semántico
- Agente con refinamiento iterativo en la misma sesión

Ejemplos de queries que resuelve:
- "Dame los mejores de fitness en Medellín con más de 3% de engagement"
- "Encuentra perfiles similares a @usuario123 en México"
- "Creadores de gastronomía en Bogotá con buen historial de campañas y sin campañas de alcohol en los últimos 6 meses"

**Restricción de privacidad (no negociable en ninguna fase):** teléfonos y emails nunca entran en vectores ni en contexto de prompts. Solo accesibles en la capa de aplicación con control de acceso por rol.
